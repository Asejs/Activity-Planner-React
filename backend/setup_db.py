...
def get_all_activities(conn):
    ...
    try:
        sql = (
            ...
            "created FROM activities ORDER BY created DESC"
        )
        cur.execute(sql)
        activities = []
        for row in cur:
...
def get_all_activities_date(conn):
    ...
    try:
        sql = (
            "SELECT username, activity_id, title, date, location, description, image,"
            "(SELECT COUNT(*) FROM participations WHERE participations.activity_id = activities.activity_id) AS participants,"
            "created FROM activities ORDER BY date"
        )
        cur.execute(sql)
        activities = []
        for row in cur:
...
if __name__ == "__main__":
    try:
        ...
        # add new activities:
        add_new_activity(conn, "aase", "Joggetur rundt Mosvannet",  "2022-12-15", "Stavanger", "Bli med på en rolig joggetur rundt Mosvannet.\n\nStarter kl 10 om morgenen.", "jogge.jpg")
        add_new_activity(conn, "maribj", "Tur til Preikestolen",  "2022-12-16", "Preikestolen", "SIS arrangerer tur til Preikestolen.\n\nMer informasjon kommer!", "preikestolen.jpg")
        add_new_activity(conn, "livesa", "Digital Paint'n Sip",  "2022-12-15", "Digitalt", "DIGITAL PAINT'N SIP \n\n Har du lyst til å bli med på Paint'n Sip den 15.desember? \n Meld deg på! \n\n Du får tilsendt maling og lerret.", "paintnsip.png")
        add_new_activity(conn, "jorgen", "Grilling",  "2023-06-06", "Godalen", "Grilling ved Godalen", "grill.jpg")
